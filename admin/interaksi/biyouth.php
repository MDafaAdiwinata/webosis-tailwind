...
// Bangun query
$sql = "SELECT * FROM tb_biyouth WHERE 1=1";

if ($search != '') {
    $sql .= " AND (id_biyouth LIKE '%$search%' or judul_biyouth LIKE '%$search%' or nama_peserta LIKE '%$search%' OR gambar_biyouth LIKE '%$search%')";
}
if ($urutkan_data == 'terbaru') {
    $sql .= " ORDER BY id_biyouth ASC";
} elseif ($urutkan_data == 'terlama') {
    $sql .= " ORDER BY id_biyouth DESC";
}

// Eksekusi query
$ambilBiyouth = mysqli_query($koneksi, $sql);

// Delete Data
if (isset($_GET['kode'])) {
    $id_biyouth = mysqli_real_escape_string($koneksi, $_GET['kode']);

    // Ambil nama file gambar untuk dihapus dari server
    $result = mysqli_query($koneksi, "SELECT gambar_biyouth FROM tb_biyouth WHERE id_biyouth='$id_biyouth'");
    $data   = mysqli_fetch_array($result);

    if ($data) {
        $gambar_untuk_dihapus = $data['gambar_biyouth'];
        $file_path = "../../assets/img/biyouth_creation/" . $gambar_untuk_dihapus;
    ...
    mysqli_query($koneksi, "DELETE FROM tb_biyouth WHERE id_biyouth='$id_biyouth'");
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-6 sm:gap-10 rounded-2xl">
                ...
                <div class="flex flex-col items-end justify-center">
                    <a href="../crud/tambah-biyouth.php"
                ...
                <div
                    ...
                    <table class="w-full text-md text-left rtl:text-right text-[var(--txt-primary2)] border-collapse">
                        ...
                        <tbody>
                            ...
                            <?php
                            if ($ambilBiyouth && mysqli_num_rows($ambilBiyouth) > 0) {
                            ...
                            ?>
                                    ...
                                    <tr
                                        ...
                                        <th scope="row"
                                            ...
                                            <?= htmlspecialchars($tampilBiyouth['id_biyouth']); ?>
                                        </th>
                                        <td class="px-6 py-4">
                                            <?= htmlspecialchars($tampilBiyouth['judul_biyouth']); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?= htmlspecialchars($tampilBiyouth['nama_peserta']); ?>
                                        </td>
                                        <td class="px-6 py-4" width="20%">
                                            <img src="../../assets/img/biyouth_creation/<?= htmlspecialchars($tampilBiyouth['gambar_biyouth']); ?>"
                                                alt="Gambar Biyouth">
                                        </td>
                                        <td class="px-6 py-4 text-right space-y-2">
                                            <a href="../crud/ubah-biyouth.php?kode=<?= $tampilBiyouth['id_biyouth']; ?>" class="font-medium text-md sm:text-lg text-yellow-600 hover:underline">
                                                UBAH
                                            </a>
                                            <button data-modal-target="modalHapusBiyouth<?= $tampilBiyouth['id_biyouth']; ?>" data-modal-toggle="modalHapusBiyouth<?= $tampilBiyouth['id_biyouth']; ?>" class="font-medium text-md lg:text-lg text-red-600 hover:underline">
                                                HAPUS
                                            </button>
                                        </td>
                                    </tr>
                                    ...
                                    <!-- Modal Data - Delete -->
                                    <div id="modalHapusBiyouth<?= $tampilBiyouth['id_biyouth']; ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <div class="relative bg-[var(--bg-primary)] rounded-lg shadow-sm">
                                                <button type="button" class="absolute top-3 end-2.5 text-[var(--txt-primary)]/50 bg-transparent hover:bg-[var(--txt-primary)]/30 hover:text-[var(--txt-primary)]/80 rounded-xl text-sm w-8 h-8 ms-auto inline-flex justify-center items-center cursor-pointer" data-modal-hide="modalHapusBiyouth<?= $tampilBiyouth['id_biyouth']; ?>">
                                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                                    </svg>
                                                    <span class="sr-only">Close modal</span>
                                                ...
                                                <div class="p-4 md:p-5 text-center">
                                                    ...
                                                    <button data-modal-hide="modalHapusBiyouth<?= $tampilBiyouth['id_biyouth']; ?>" type="button" class="cursor-pointer py-2.5 px-5 text-sm font-medium text-[var(--txt-primary)] focus:outline-none bg-[var(--bg-secondary3)]/0 rounded-lg border border-[var(--bg-secondary3)]/30 hover:bg-[var(--bg-secondary3)]/10 hover:text-[var(--txt-primary)] focus:z-10 ">
                                                        Cancel
                                                    </button>
                                                    <a href="?kode=<?= $tampilBiyouth['id_biyouth']; ?>" class="ms-2 text-[var(--txt-primary)] bg-[var(--text-danger)]/80 hover:bg-[var(--text-danger)] font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center cursor-pointer">
                                                        Hapus
                                                    </a>
                                                </div>
                                            </div>
