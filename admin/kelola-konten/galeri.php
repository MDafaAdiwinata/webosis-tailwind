...
$kategori_galeri = isset($_GET['kategori_galeri']) ? $_GET['kategori_galeri'] : '';
...
if ($kategori_galeri != '') {
    $sql .= " AND kategori_galeri = '$kategori_galeri'";
}
if ($search != '') {
    $sql .= " AND (id_galeri LIKE '%$search%' or kategori_galeri LIKE '%$search%' or img_galeri LIKE '%$search%')";
}
if ($urutkan_data == 'terbaru') {
    $sql .= " ORDER BY id_galeri ASC";
} elseif ($urutkan_data == 'terlama') {
    $sql .= " ORDER BY id_galeri DESC";
}

// Eksekusi query
$ambilGaleri = mysqli_query($koneksi, $sql);

// Delete Data
if (isset($_GET['kode'])) {
    $id_galeri = mysqli_real_escape_string($koneksi, $_GET['kode']);

    // Ambil nama file gambar untuk dihapus dari server
    $result = mysqli_query($koneksi, "SELECT img_galeri FROM tb_galeri WHERE id_galeri='$id_galeri'");
    $data   = mysqli_fetch_array($result);

    if ($data) {
        $gambar_untuk_dihapus = $data['img_galeri'];
    ...
    mysqli_query($koneksi, "DELETE FROM tb_galeri WHERE id_galeri='$id_galeri'") or die(mysqli_error($koneksi));
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-6 sm:gap-10 rounded-2xl">
                <form class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                    <div class="max-w-2xl">
                        <select id="countries" name="kategori_galeri" onchange="this.form.submit()"
                            ...
                            <option value="">Kategori Gambar</option>
                            <option value="Landscape" <?= ($kategori_galeri == 'Landscape') ? 'selected' : '' ?>>Landscape</option>
                            <option value="Kotak" <?= ($kategori_galeri == 'Kotak') ? 'selected' : '' ?>>Kotak</option>
                        </select>
                    </div>
                    <div class="max-w-2xl">
                        <select id="countries" name="urutkan_data" onchange="this.form.submit()"
                            class="bg-transparent border border-txt-primary2/50 text-txt-primary2 text-md md:text-lg rounded-2xl focus:ring-bg-primary focus:border-bg-primary block w-full px-4 cursor-pointer hover:bg-[var(--bg-primary)]/10 transition duration-300">
                ...
                <div
                    ...
                    <table class="w-full text-md text-left rtl:text-right text-[var(--txt-primary2)] border-collapse">
                        ...
                        <tbody>
                            ...
                            <?php
                            if ($ambilGaleri && mysqli_num_rows($ambilGaleri) > 0) {
                                while ($tampilGaleri = mysqli_fetch_array($ambilGaleri)) {
                            ?>
                                    ...
                                    <tr
                                        ...
                                        <th scope="row"
                                            ...
                                            <?= htmlspecialchars($tampilGaleri['id_galeri']); ?>
                                        </th>
                                        <td class="px-6 py-4">
                                            <?= htmlspecialchars($tampilGaleri['kategori_galeri']); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <img src="../../assets/img/galeri/<?= htmlspecialchars($tampilGaleri['img_galeri']); ?>"
                                                alt="Galeri">
                                        </td>
                                        <td class="px-6 py-4 text-right space-y-2">
                                            <a href="../crud/ubah-galeri.php?kode=<?= $tampilGaleri['id_galeri']; ?>" class="font-medium text-md sm:text-lg text-yellow-600 hover:underline">
                                                UBAH
                                            </a>
                                            <button data-modal-target="modalHapusGaleri<?= $tampilGaleri['id_galeri']; ?>" data-modal-toggle="modalHapusGaleri<?= $tampilGaleri['id_galeri']; ?>" class="font-medium text-md lg:text-lg text-red-600 hover:underline">
                                                HAPUS
                                            </button>
                                        </td>
                                    </tr>
                                    ...
                                    <!-- Modal Data - Delete -->
                                    <div id="modalHapusGaleri<?= $tampilGaleri['id_galeri']; ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <div class="relative bg-[var(--bg-primary)] rounded-lg shadow-sm">
                                                <button type="button" class="absolute top-3 end-2.5 text-[var(--txt-primary)]/50 bg-transparent hover:bg-[var(--txt-primary)]/30 hover:text-[var(--txt-primary)]/80 rounded-xl text-sm w-8 h-8 ms-auto inline-flex justify-center items-center cursor-pointer" data-modal-hide="modalHapusGaleri<?= $tampilGaleri['id_galeri']; ?>">
                                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                                    </svg>
                                                    <span class="sr-only">Close modal</span>
                                                ...
                                                <div class="p-4 md:p-5 text-center">
                                                    ...
                                                    <button data-modal-hide="modalHapusGaleri<?= $tampilGaleri['id_galeri']; ?>" type="button" class="cursor-pointer py-2.5 px-5 text-sm font-medium text-[var(--txt-primary)] focus:outline-none bg-[var(--bg-secondary3)]/0 rounded-lg border border-[var(--bg-secondary3)]/30 hover:bg-[var(--bg-secondary3)]/10 hover:text-[var(--txt-primary)] focus:z-10 ">
                                                        Cancel
                                                    </button>
                                                    <a href="galeri.php?kode=<?= $tampilGaleri['id_galeri']; ?>" class="ms-2 text-[var(--txt-primary)] bg-[var(--text-danger)]/80 hover:bg-[var(--text-danger)] font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center cursor-pointer">
                                                        Hapus
                                                    </a>
                                                </div>
                                            </div>
