...
if ($search != '') {
    $sql .= " AND (id_news LIKE '%$search%' or judul_news LIKE '%$search%' or deskripsi_news LIKE '%$search%' or gambar LIKE '%$search%')";
}
if ($urutkan_data == 'terbaru') {
    $sql .= " ORDER BY id_news ASC";
} elseif ($urutkan_data == 'terlama') {
    $sql .= " ORDER BY id_news DESC";
}

// Eksekusi query
$ambilNews = mysqli_query($koneksi, $sql);

// Delete Data
if (isset($_GET['kode'])) {
    $id_news = mysqli_real_escape_string($koneksi, $_GET['kode']);

    // Ambil nama file gambar untuk dihapus dari server
    $result = mysqli_query($koneksi, "SELECT gambar FROM tb_news WHERE id_news='$id_news'");
    $data   = mysqli_fetch_array($result);

    if ($data) {
        $gambar_untuk_dihapus = $data['gambar'];
        $file_path = "../../assets/img/news/" . $gambar_untuk_dihapus;
    ...
    mysqli_query($koneksi, "DELETE FROM tb_news WHERE id_news='$id_news'") or die(mysqli_error($koneksi));
                ...
                <div
                    ...
                    <table class="w-full text-md text-left rtl:text-right text-[var(--txt-primary2)] border-collapse">
                        ...
                        <tbody>
                            ...
                            <?php
                            if ($ambilNews && mysqli_num_rows($ambilNews) > 0) {
                            ...
                            ?>
                                    ...
                                    <tr
                                        ...
                                        <th scope="row"
                                            ...
                                            <?= htmlspecialchars($tampilNews['id_news']); ?>
                                        </th>
                                        <td class="px-6 py-4">
                                            <img src="../../assets/img/news/<?= htmlspecialchars($tampilNews['gambar']); ?>"
                                                alt="Dummy Img">
                                        </td>
                                        <td class="px-6 py-4">
                                            <?= htmlspecialchars($tampilNews['judul_news']); ?>
                                        </td>
                                        <td class="px-6 py-4">
                                            <?= htmlspecialchars($tampilNews['deskripsi_news']); ?>
                                        </td>
                                        <td class="px-6 py-4 text-right space-y-2">
                                            <a href="../crud/ubah-news.php?kode=<?= $tampilNews['id_news']; ?>" class="font-medium text-md sm:text-lg text-yellow-600 hover:underline">
                                                UBAH
                                            </a>
                                            <button data-modal-target="modalHapusNews<?= $tampilNews['id_news']; ?>" data-modal-toggle="modalHapusNews<?= $tampilNews['id_news']; ?>" class="font-medium text-md lg:text-lg text-red-600 hover:underline">
                                                HAPUS
                                            </button>
                                        </td>
                                    </tr>
                                    ...
                                    <!-- Modal Data - Delete -->
                                    <div id="modalHapusNews<?= $tampilNews['id_news']; ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                        <div class="relative p-4 w-full max-w-md max-h-full">
                                            <div class="relative bg-[var(--bg-primary)] rounded-lg shadow-sm">
                                                <button type="button" class="absolute top-3 end-2.5 text-[var(--txt-primary)]/50 bg-transparent hover:bg-[var(--txt-primary)]/30 hover:text-[var(--txt-primary)]/80 rounded-xl text-sm w-8 h-8 ms-auto inline-flex justify-center items-center cursor-pointer" data-modal-hide="modalHapusNews<?= $tampilNews['id_news']; ?>">
                                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                                    </svg>
                                                    <span class="sr-only">Close modal</span>
                                                ...
                                                <div class="p-4 md:p-5 text-center">
                                                    ...
                                                    <button data-modal-hide="modalHapusNews<?= $tampilNews['id_news']; ?>" type="button" class="cursor-pointer py-2.5 px-5 text-sm font-medium text-[var(--txt-primary)] focus:outline-none bg-[var(--bg-secondary3)]/0 rounded-lg border border-[var(--bg-secondary3)]/30 hover:bg-[var(--bg-secondary3)]/10 hover:text-[var(--txt-primary)] focus:z-10 ">
                                                        Cancel
                                                    </button>
                                                    <a href="news.php?kode=<?= $tampilNews['id_news']; ?>" class="ms-2 text-[var(--txt-primary)] bg-[var(--text-danger)]/80 hover:bg-[var(--text-danger)] font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center cursor-pointer">
                                                        Hapus
                                                    </a>
                                                </div>
                                            </div>
