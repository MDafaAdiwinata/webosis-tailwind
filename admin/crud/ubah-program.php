...
// Panggil Data
$ambilProgram = mysqli_query($koneksi, "SELECT * FROM tb_promdat WHERE id_promdat='$_GET[kode]'");
$tampilProgram = mysqli_fetch_array($ambilProgram);
...
if (isset($_POST['ubah'])) {
    $id_promdat = $_POST['id_promdat'];
    $judul = mysqli_real_escape_string($koneksi, $_POST['judul_card']);
    $tanggal = mysqli_real_escape_string($koneksi, $_POST['tanggal_card']);
    $kategori = mysqli_real_escape_string($koneksi, $_POST['kategori']);
    $deskripsi = mysqli_real_escape_string($koneksi, $_POST['deskripsi_card']);

    $gambar_lama = $_POST['gambar_lama']; // dari input hidden
    $gambar_baru = $_FILES['img_card_update']['name'];
    $tmp = $_FILES['img_card_update']['tmp_name'];
    $ukuran = $_FILES['img_card_update']['size'];
    $error = $_FILES['img_card_update']['error'];

    // Lokasi folder tujuan - pastikan path benar
    $folder = '../../assets/img/promdat/';
    ...
    if (!empty($gambar_baru) && $error === 0) {
        ...
        if (!in_array($ekstensi, $ekstensi_valid)) {
            ...
            header("Location: ubah-program.php?kode=$id_promdat");
            exit;
        }
        ...
        if ($ukuran > 2 * 1024 * 1024) {
            ...
            header("Location: ubah-program.php");
            exit;
        }
        ...
        if (move_uploaded_file($tmp, $pathBaru)) {
            ...
            $query = "UPDATE tb_promdat SET 
                      judul_card='$judul', 
                      tanggal_card='$tanggal', 
                      kategori='$kategori', 
                      deskripsi_card='$deskripsi', 
                      img_card='$newName'
                      WHERE id_promdat='$id_promdat'";
            ...
            <span class="sr-only">Info</span>
                <div class="ms-3 me-4 text-md font-medium">
                    Data berhasil ditambahkan dengan gambar baru!
                </div>
            <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-[var(--bg-success)]/30 text-[var(--text-success)] rounded-lg cursor-pointer focus:ring-2 p-1.5 transition duration-300 border border-[var(--bg-success)] inline-flex items-center justify-center h-8 w-8" data-dismiss-target="#alert-2" aria-label="Close">
                ...
                header("Location: ../kelola-konten/program.php");
                exit;
            } else {
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex w-full items-center justify-center mt-4">
                <form class="space-y-4 w-full lg:w-2/5 mb-32" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="id_promdat" value="<?= $tampilProgram['id_promdat']; ?>">
                    <input type="hidden" name="gambar_lama" value="<?= $tampilProgram['img_card']; ?>">
                    <div class="mb-6">
                        <?php echo $alert_message; ?>
                    </div>
                    <div class="mb-6">
                        <small class="block text-sm text-[var(--txt-primary2)]">Gambar saat ini:
                            <?= $tampilProgram['img_card']; ?>
                        </small>
                        <?php if (!empty($tampilProgram['img_card']) && file_exists("../../assets/img/promdat/" . $tampilProgram['img_card'])): ?>
                            <img src="../../assets/img/promdat/<?= $tampilProgram['img_card']; ?>" alt="Preview Gambar" width="300"
                    ...
                    <div class="mb-6">
                        ...
                        <input
                            ...
                            id="img_card_update" type="file" accept=".jpg, .png, .jpeg" name="img_card_update">
                    </div>
                    <div class="mb-6">
                        <label for="judul_card"
                            class="block mb-2 text-lg font-normal text-[var(--txt-primary2)]">
                            Judul Program
                        </label>
                        <input type="text" name="judul_card" id="judul_card" value="<?php echo $tampilProgram['judul_card'] ?>"
                    ...
                    <div class="mb-6">
                        <label for="tanggal_card"
                            class="block mb-2 text-lg font-normal text-[var(--txt-primary2)]">
                            Tanggal
                        </label>
                        <input type="text" name="tanggal_card" id="tanggal_card" value="<?php echo $tampilProgram['tanggal_card'] ?>"
                            class="bg-transparent border border-[var(--bg-primary)]/50 text-[var(--txt-primary2)] text-md rounded-xl focus:ring-[var(--txt-primary2)]/80 focus:border-[var(--txt-primary2)]/80 block w-full p-2.5" required />
                    </div>
                    <div class="mb-6">
                        <label for="kategori"
                        ...
                        <select id="kategori" name="kategori"
                            ...
                            $kategori_sekarang = isset($tampilProgram['kategori']) ? $tampilProgram['kategori'] : '';
                            $kategori_options = ['Pendidikan', 'Agama', 'Minat dan Bakat', 'Kesehatan', 'Negara'];
                    ...
                    <div>
                        ...
                        <textarea id="deskripsiTentang" rows="4" name="deskripsi_card"
                            class="bg-transparent border border-[var(--bg-primary)]/50 text-[var(--txt-primary2)] text-md rounded-xl focus:ring-[var(--txt-primary2)]/80 focus:border-[var(--txt-primary2)]/80 block w-full p-2.5"><?php echo $tampilProgram['deskripsi_card'] ?></textarea>
                    </div>
