...
// Panggil Data
$ambilMainImg = mysqli_query($koneksi, "SELECT * FROM tb_main_content_img WHERE id_content_img='$_GET[kode]'");
$tampilMainImg = mysqli_fetch_array($ambilMainImg);
...
if (isset($_POST['ubah'])) {
    $id_content_img = $_POST['id_content_img'];
    $nama_img = mysqli_real_escape_string($koneksi, $_POST['nama_img']);

    $gambar_lama = $_POST['gambar_lama']; // dari input hidden
    $gambar_baru = $_FILES['img_content_update']['name'];
    $tmp = $_FILES['img_content_update']['tmp_name'];
    $ukuran = $_FILES['img_content_update']['size'];
    $error = $_FILES['img_content_update']['error'];
    ...
    if (!empty($gambar_baru) && $error === 0) {
        ...
        if (!in_array($ekstensi, $ekstensi_valid)) {
            ...
            header("Location: ubah-content-img.php?kode=$id_content_img");
            exit;
        }
        ...
        if (move_uploaded_file($tmp, $pathBaru)) {
            ...
            $query = "UPDATE tb_main_content_img SET 
                      nama_img='$nama_img',
                      img_content='$newName'
                      WHERE id_content_img='$id_content_img'";
            ...
            <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-[var(--bg-danger)]/30 text-[var(--text-danger)] rounded-lg cursor-pointer focus:ring-2 p-1.5 transition duration-300 border border-[var(--bg-danger)] inline-flex items-center justify-center h-8 w-8" data-dismiss-target="#alert-2" aria-label="Close">
                ...
                header("Location: ubah-content-img.php");
                exit;
            }
        } else {
            $_SESSION['alert_message'] = '
        ...
        if (mysqli_query($koneksi, $query)) {
            ...
            header("Location: ../portal-lomba/main-content.php?kode=$id_divisi");
            exit;
        }
    }
}
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex w-full items-center justify-center mt-4">
                <form class="space-y-4 w-full lg:w-2/5 mb-32" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="id_content_img" value="<?= $tampilMainImg['id_content_img']; ?>">
                    <input type="hidden" name="gambar_lama" value="<?= $tampilMainImg['img_content']; ?>">
                    <div class="mb-6">
                        <?php echo $alert_message; ?>
                    </div>
                    <div class="mb-6">
                        <small class="block text-sm text-[var(--txt-primary2)]">Gambar saat ini:
                            <?= $tampilMainImg['img_content']; ?>
                        </small>
                        <?php if (!empty($tampilMainImg['img_content']) && file_exists("../../assets/img/galeri/" . $tampilMainImg['img_content'])): ?>
                            <img src="../../assets/img/galeri/<?= $tampilMainImg['img_content']; ?>" alt="Preview Gambar" width="400"
                                class="mt-2 img-thumbnail d-block">
                        <?php endif; ?>
                    </div>
                    <div class="mb-6">
                        <label class="block mb-2 text-lg font-normal text-[var(--txt-primary2)]"
                            for="img_content_update">
                        ...
                        <input
                            ...
                            id="img_content_update" type="file" accept=".jpg, .png, .jpeg" name="img_content_update">
                    </div>
                    <div class="mb-6">
                        <label for="nama_img"
                        ...
                        <input type="text" name="nama_img" id="nama_img" value="<?php echo $tampilMainImg['nama_img'] ?>"
