...
$ambilUser = mysqli_query($koneksi, "SELECT * FROM tb_user WHERE id_user='$_GET[kode]'");
$tampilUser = mysqli_fetch_array($ambilUser);

// Proses Update Data User - DIPERBAIKI dengan HASHING
if (isset($_POST['ubah'])) {
    ...
    $check_query = "SELECT * FROM tb_user WHERE username = '$username' AND id_user != '$id_user'";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        header("Location: ubah-akun-kelas.php?kode=" . $id_user);
        exit;
    }

    // Logika update password - HANYA update jika diisi
    if (!empty($password)) {
        // Hash password baru menggunakan password_hash
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);

        // Jika password diisi, update username dan password (dengan hashing)
    ...
    if (mysqli_query($koneksi, $query)) {
        ...
        <div id="alert-2" class="flex items-center p-4 text-[var(--text-success)] rounded-2xl bg-[var(--bg-success)]" role="alert">
            ...
            <span class="sr-only">Info</span>
                <div class="ms-3 me-4 text-sm md:text-md font-medium">
                    Data Akun Kelas berhasil diperbarui.
                </div>
        ...
        header("Location: ../portal-lomba/akun-kelas.php");
        exit;
    } else {
        $_SESSION['alert_message'] = '
        <div id="alert-2" class="flex items-center p-4 text-[var(--text-danger)] rounded-2xl bg-[var(--bg-danger)]" role="alert">
            ...
            <span class="sr-only">Info</span>
                <div class="ms-3 me-4 text-sm md:text-md font-medium">
                    Data Akun Kelas gagal diperbarui. (Harap Coba lagi). Gagal terus? hubungi operator/admin!
                </div>
            <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-[var(--bg-danger)]/30 text-[var(--text-danger)] rounded-lg cursor-pointer focus:ring-2 p-1.5 transition duration-300 border border-[var(--bg-danger)] inline-flex items-center justify-center h-8 w-8" data-dismiss-target="#alert-2" aria-label="Close">
                <span class="sr-only">Close</span>
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
...
<head>
    ...
    <title>Ubah Akun Kelas - Admin Dashboard</title>
...
<body class="font-[montserrat]">
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex w-full items-center justify-center mt-4">
                <form class="space-y-4 w-full lg:w-2/5 mb-32" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="id_user" value="<?= $tampilUser['id_user']; ?>">

                    <div class="mb-6">
                        <label for="username" class="block mb-2 text-lg font-normal text-[var(--txt-primary2)]">
                            Username
                        </label>
                        <input type="text" name="username" id="username"
                            value="<?php echo htmlspecialchars($tampilUser['username']); ?>"
                    ...
                    <div class="mb-6">
                        <label for="password" class="block mb-2 text-lg font-normal text-[var(--txt-primary2)]">
                            Password Baru
                        </label>
                        <input type="password" name="password" id="password"
                            placeholder="Masukkan password baru"
                            class="bg-transparent border border-[var(--bg-primary)]/50 text-[var(--txt-primary2)] text-md rounded-xl focus:ring-[var(--txt-primary2)]/80 focus:border-[var(--txt-primary2)]/80 block w-full p-2.5" />
                        <p class="mt-2 text-sm text-[var(--txt-primary2)]/60">
                            Biarkan kosong jika tidak ingin mengubah password. Password akan di-hash secara otomatis.
                        </p>
                    </div>

                    <div class="mb-6">
                        <label for="confirm_password" class="block mb-2 text-lg font-normal text-[var(--txt-primary2)]">
    ...
    <!-- Script untuk validasi konfirmasi password -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                // Jika password diisi, maka harus ada konfirmasi password
            ...
            // Jika password kosong, kosongkan juga confirm password dan nonaktifkan validasi
